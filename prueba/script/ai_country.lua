function CountryAI_Tick(country)
    ...
    if country:GetResource("manpower") >= 1500 and country:GetResource("money") >= 3000 then
        ...
        local function createBrigades(unitType, requiredBrigades)
            local currentBrigades = country:GetNumberOfRegimentsOfType(unitType)
        ...
        -- Unir las brigadas en ejércitos de tamaño 30
        local function mergeBrigades(unitType, requiredBrigades)
            local totalBrigades = country:GetNumberOfRegimentsOfType(unitType)
            local remainingBrigades = totalBrigades
            while remainingBrigades >= 30 do
                local unitsToMerge = {}
                for i = 1, 4 do -- Agregar 4 de infantería
                    table.insert(unitsToMerge, country:GetRegimentOfType("infantry"))
                end
                for i = 1, 1 do -- Agregar 1 de caballería
                    table.insert(unitsToMerge, country:GetRegimentOfType("cuirassier"))
                end
                for i = 1, 5 do -- Agregar 5 de artillería
                    table.insert(unitsToMerge, country:GetRegimentOfType("artillery"))
                end
                country:MergeUnits(unitsToMerge)
                remainingBrigades = remainingBrigades - 30
            end
        end

        -- Unir brigadas en ejércitos de tamaño 30
        mergeBrigades("infantry", 4)
        mergeBrigades("cuirassier", 1)
        mergeBrigades("artillery", 5)
...
-- Función para liberar una nación
function liberateNation(country, target)
    local provinceList = target:GetCoreProvinces()
    for i = 0, provinceList:Size() - 1 do
        local province = provinceList:GetAt(i)
        target:TakeCore(province)
        province:SetController(target)
    end
    target:GetCountry():ChangeGovernmentIdeology("democracy")
    target:GetCountry():AddPrestige(50)
    target:GetCountry():AddTechResearched()
    target:GetCountry():ChangeOpinion(country, 50)
end
