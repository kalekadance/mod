...
country = {
	...
	GREATNESS_DAYS = 395, 	 -- how many days until country risks losing status as great nation
	BADBOY_LIMIT = 125,
	...
	INVESTMENT_SCORE_FACTOR = 0.0012, -- how much foreign investment money counts towards your industry score
	...
	NAVAL_BASE_SUPPLY_SCORE_BASE = 30, -- base value that is powered by level of naval base. Determines the naval supplying capabilities.
	...
	COLONIZATION_PROTECTORATE_PROVINCE_MAINTAINANCE = 5,
	...
	COLONIZATION_INTEREST_COST_INITIAL = 400,
	COLONIZATION_INTEREST_COST_NEIGHBOR_MODIFIER = -100,
	...
	MAX_RESEARCH_POINTS = 60000, -- max RP you can store for uncivs
},
...
economy = {
	...
	FACTORY_PURCHASE_DRAWDOWN_FACTOR = 0.015		-- the % a factory will reduce its input purchases each day if it did not sell all its goods (also used for scaling up production if all goods are sold)
},
...
military = {
	...
	POP_SIZE_PER_REGIMENT = 4500,
	SOLDIER_TO_POP_DAMAGE = 0.08,
	LAND_SPEED_MODIFIER = 1,
	NAVAL_SPEED_MODIFIER = 2,
	EXP_GAIN_DIV = 0.07,
	LEADER_RECRUIT_COST = 5,
	SUPPLY_RANGE = 250,
	POP_MIN_SIZE_FOR_REGIMENT_PROTECTORATE_MULTIPLIER = 9,
	POP_MIN_SIZE_FOR_REGIMENT_COLONY_MULTIPLIER = 8,
	...
	LEADER_PRESTIGE_TO_MORALE_FACTOR = 1.2, -- f.ex. 100% of prestige = +10% morale
	LEADER_PRESTIGE_TO_MAX_ORG_FACTOR = 1.1, -- f.ex. 100% of prestige = +10 max org
	...
	AI_TRANSPORT_PROPORTION = 0.9, -- fraction of ships in the navy that should be should be transports
	AI_CAVALRY_PROPORTION = 0.0, -- fraction of brigades that should be cavalry
	...
	AI_ESCORT_RATIO = 0.0, -- ratio of escorts to transports in invasion fleets
	AI_ARMY_TAXBASE_FRACTION = 0.99, -- max of tax base that AI will spend on army supply costs (based on peacetime costs)
	AI_NAVY_TAXBASE_FRACTION = 0.99, -- max of tax base that AI will spend on navy supply costs (based on peacetime costs)
	AI_BLOCKADE_RANGE = 2750, -- max distance the AI will send out blockade fleets from their home base
	RECON_UNIT_RATIO = 0.1, -- the % of units in the army that must have a recon value to get the full bonus
	ENGINEER_UNIT_RATIO = 0.0, -- the % of units in the army that must have a fort attack value to get the full bonus
	...
	NAVAL_LOW_SUPPLY_DAMAGE_MIN_STR = 0.0, -- when low supply, the navy will supply STR damage but no less then X% to avoid destruction (value from 0 to 100.0)
...
diplomacy = {
	...
	PRESTIGE_CLEAR_UNION_SPHERE_BASE = 5, -- Prestige for asserting hegemony
	PRESTIGE_GUNBOAT_BASE = 0.2, -- Prestige for debt collection
	...
	WARGOAL_JINGOISM_REQUIREMENT = 0.035,
...
pops = {
	...
	ASSIMILATION_SCALE = 0.0045,
	CONVERSION_SCALE = 0.015,
	IMMIGRATION_SCALE = 0.0065,
	...
	MOVEMENT_SUPPORT_UH_FACTOR = 4, -- 3x means 30% country support equals full UH support
...
ai =
{
	...
	ADMINISTRATOR_WEIGHT = 100.0, -- ai weight for new bureaucrat
	INDUSTRYWORKER_WEIGHT = 30.0, -- ai weight for new industry workers
	EDUCATOR_WEIGHT = 100.0, -- ai weigth for new clergy
	SOLDIER_WEIGHT = 6.0, -- ai weight for soldiers
	SOLDIER_FRACTION = 0.06, -- max amount of population AI wants to be soldiers
	CAPITALIST_FRACTION = 0.0001, -- max amount of population AI wants to be capis
	PRODUCTION_WEIGHT = 0.05, -- ai weight for new production
	SPAM_PENALTY = 10, -- makes certain diplomatic action less common 
	ONE_SIDE_MAX_WARSCORE = 99, -- don't add too many wargoals to one side in a war
	...
	CHANCE_BUILD_NAVAL_BASE = 0.4,
	CHANCE_BUILD_FORT = 0.001,
	CHANCE_INVEST_POP_PROJ = 0.50,
	CHANCE_FOREIGN_INVEST = 0.36,
	TWS_AWARENESS_SCORE_LOW_CAP = 0.2, -- AI will always add CBs if current warscore is less then that number (including TWS)
	TWS_AWARENESS_SCORE_ASPECT = 0.8, -- AI will not add any more CBs when TWS is more then X% of total WS. (to not destroy the progress)
	...
	PEACE_ALLY_BASE_RELUCTANCE_MULT = 3.0, -- Multiplies PEACE_BASE_RELUCTANCE for allies in a war
